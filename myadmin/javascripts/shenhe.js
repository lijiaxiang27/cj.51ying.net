$(function(){
		...
		// 签到记录姓名的开关2表示不记录 ，1 记录
		$('.name-switch').on('switch-change',function (e,data) {
			var $el = $(data.el), value = data.value;
			if(value){
				$.get(
					"shenhe.do.php",
					{do:"nameswitch",switchname:$el[0].name,cid:"1"},
					function (json){}
				);
			}else{
				$.get(
					"shenhe.do.php",
					{do:"nameswitch",switchname:$el[0].name,cid:"2"},
					function (json){}
				);
			}
		});
		...
		//签到记录电话的开关
		$('.phone-switch').on('switch-change',function (e,data) {
			var $el = $(data.el), value = data.value;
			if(value){
				$.get(
					"shenhe.do.php",
					{do:"phoneswitch",switchname:$el[0].name,cid:"1"},
					function (json){}
				);
			}else{
				$.get(
					"shenhe.do.php",
					{do:"phoneswitch",switchname:$el[0].name,cid:"2"},
					function (json){}
				);
			}
		});
		...
		$('#cj_condition').on('switch-change', function (e, data) {
			var $el = $(data.el), value = data.value;
		   if(value){
			   $.get("shenhe.do.php", {do: "cjcondition", cid: "2" },function(){
				   });
			   }else{
			   $.get("shenhe.do.php", {do: "cjcondition", cid: "1" },function(){
				   });
				   }
		});
		$('.memcache-switch').on('switch-change',function (e,data) {
			var $el = $(data.el), value = data.value;
			if(value){
				$.get(
					"shenhe.do.php",
					{do:"memcacheswitch",switchname:$el[0].name,cid:"1"},
					function (json){}
				);
			}else{
				$.get(
					"shenhe.do.php",
					{do:"memcacheswitch",switchname:$el[0].name,cid:"2"},
					function (json){}
				);
			}
		});
