...
$flag_m=new M('flag');
$data=$flag_m->select(' flag>1 order by id desc ');
// $newdata=array();
...
$objPHPExcel->getProperties()->setCreator("微赢科技")
							 ...
							 ->setTitle("Office 2007 XLSX 微信上墙签到用户列表")
							 ->setSubject("Office 2007 XLSX 微信上墙签到用户列表")
							 ->setDescription("微信上墙签到用户列表.")
							 ->setKeywords("微信上墙签到用户列表")
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('微信上墙签到用户列表');
...
// $filename='微信上墙签到用户列表_导出时间_'.$time_str.'.xlsx';
// Redirect output to a client鈥檚 web browser (Excel2007)
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
// header('Content-Disposition: attachment;filename="微信上墙签到用户列表_导出时间.xlsx"');
header('Content-Disposition: attachment;filename="微信上墙签到用户列表.xlsx"');
