$(function () {
    ...
    function cj_ready() {
        ...
        cj_per = new Array();
        $.getJSON('cj_plug/cj_data.php?action=ready&shady=' + cj_shady, function (json) {

            if (json && json.count>0) {
                $.each(json.data, function (i, v) {
                    cj_per.push(new Array(v['id'], v['avatar'], v['nickname'], v['shady'], v['from']));
                });
                $(".lotteryUserNum").html(json.count);
    ...
    function cj_stop() {
        ...
        $('.lotteryUserNum').text($(".lotteryUserNum").text()-1);
        cjstart_btn.hide();
        var cjcache_bin = cjstart_btn;
        cjstart_btn = '';
        $.post("cj_plug/cj_data.php?action=ok", {id: cj_mid, grade: cj_shady}, function (msg) {
            if (msg == 1) {
                cj_jadge = 1;
