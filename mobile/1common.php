...
if($wall_config['rentweixin']==1){//使用用户自己的公众号
	...
	if(!isset($_GET['rentopenid'])){//如果还没有获取到openid
		if (empty($_GET['code'])) {//还没有获取到code
            header("location:http://a.aotuedu.com/get-weixin-code.html?appid=".$weixin_config['appid']."&scope=snsapi_userinfo&state=hello-world&redirect_uri=http%3A%2F%2Fhd.51ying.net%2Fmobile%2Findex.php");
//			$fromurl=$currenturl;
